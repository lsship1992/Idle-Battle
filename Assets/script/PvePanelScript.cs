...
public class PvePanelScript : MonoBehaviour
{
    public int CountPve;
    public Text CountPveText;
    public GameObject UserAttckPrefab;
    public GameObject PerentUserAttck;
    public string mail;
    public int IDUserAttack;
    public int scoreEnemy;
    // Update is called once per frame
    public GameObject Battle;
    public GameObject Reiting;
    public bool ReitingActiv = false;
    public GameObject ReitingPerent;
    ...
    IEnumerator UdarBattleIenumerator()
    {
                                    ...
                                    if (UserPersonSpavn[a.idSlot].transform.GetChild(0).GetComponent<PersonBattleScript>().skill1Play != "")
                                    {
                                        ...
                                        if (server_info[27] == "Attack")
                                        {
                                            ...
                                            else if (server_info[25] == "3" || Convert.ToInt32(server_info[25]) == 3)
                                            {
                                                ...
                                                Debug.Log("Залупня " + ListSpeedPersonEnemy.Count);
                                        ...
                                        else if (server_info[27] == "Hill")
                                        {
                                            if (server_info[25] == "5" || Convert.ToInt32(server_info[25]) == 5)
                                            {
                                                personAllAttack.Clear();
                                                List<PersonSearchSpeed> GoalConvert = new List<PersonSearchSpeed>();
                                                foreach (var x in ListSpeedPersonUser)
                                                {
        ...
        {
            ClientSend.PvpWinUser(mail, 50, Convert.ToInt32(scoreEnemy * 0.1f), Convert.ToInt32(scoreEnemy * 0.05f));
    ...
    {
        PerentUserAttck.GetComponent<RectTransform>().sizeDelta = new Vector2(PerentUserAttck.GetComponent<RectTransform>().sizeDelta.x,PerentUserAttck.transform.childCount * 236);
        ReitingPerent.GetComponent<RectTransform>().sizeDelta = new Vector2(ReitingPerent.GetComponent<RectTransform>().sizeDelta.x, ReitingPerent.transform.childCount * 236);
        CountPveText.text = CountPve.ToString();
        if (!BattleStart)
        {
        ...
        else
        {
            ...
            if (CheckGoPersonUser == true && CheckGoPersonEnemy == true)
            {
                ...
                {
                    Debug.Log("Залупа" + ListSpeedPersonEnemy.Count);
                    Debug.Log("Залупа" + ListSpeedPersonUser.Count);
                }
                else
                {
                    ...
                    if (UserPersonidListSlot.Contains(a.idSlot))
                    {
                        foreach (var b in UserPersonSpavn[a.idSlot].transform.GetChild(0).gameObject.GetComponent<PersonBattleScript>().Debuf.ToList())
                        {
                            ...
                            if (server_info[0] == "Bleeding")
                            {
                                if (Convert.ToInt32(server_info[2]) != 0)
                                {
                                    float demage = ((UserPersonSpavn[a.idSlot].transform.GetChild(0).gameObject.GetComponent<PersonBattleScript>().Hp_info * Convert.ToInt32(server_info[1])) / 100);
    ...
    public void ReiningVoid()
    {
        while (PerentUserAttck.transform.childCount > 0)
        {
            DestroyImmediate(PerentUserAttck.transform.GetChild(0).gameObject);
        }
        while (ReitingPerent.transform.childCount > 0)
        {
            DestroyImmediate(ReitingPerent.transform.GetChild(0).gameObject);
        }
        ReitingActiv = true;
        Reiting.SetActive(true);
        ClientSend.SearchPveCount("!");
        ClientSend.SearchPvpUserReiting();
    }

    public void ReiningCloseVoid()
    {
        while (PerentUserAttck.transform.childCount > 0)
        {
            DestroyImmediate(PerentUserAttck.transform.GetChild(0).gameObject);
