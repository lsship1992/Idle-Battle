...
public class BattleKvestKlan : MonoBehaviour
{
    ...
    IEnumerator UdarBattleIenumerator()
    {
                                                ...
                                                try
                                                {
                                                    if (personAllAttack.Count > 0)
                                                    {
                                                        foreach (var AttackEnemy in personAllAttack)
                                                        {
                                                            ...
                                                            DamageAll = DamageAll + Convert.ToInt32(damage);
        ...
        else if (ListSpeedPersonUser.Count > 0 && ListSpeedPersonEnemy.Count == 0)
        {
            ...
            ClientSend.EndBattleKvestKlan(GameControll.instance.KlanUser.GetComponent<KlanUser>().NameKlan,DamageAll);
            ClientSend.WinRewerdKvestKlan(EnemyPersonSpavn[0].gameObject.transform.GetChild(0).gameObject.GetComponent<PersonBattleScript>().level);
    ...
    {
        ClientSend.GlavReitingUserKvestKlan(GameControll.instance.KlanUser.GetComponent<KlanUser>().NameKlan);
        ...
        GameControll.instance.KlanUser.GetComponent<KlanUser>().kvestKlan.GetComponent<CvestCklan>().BattlePanel.SetActive(false);


    }
    private async void Update()
    {
        Debug.Log(ListSpeedPersonEnemy.Count);
        Debug.Log(ListSpeedPersonUser.Count);
        if (!BattleStart)
        {
